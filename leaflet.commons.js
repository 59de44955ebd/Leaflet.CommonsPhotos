...
const _rotl = function(v, n) {
    return ((v << n) | (v >>> (32 - n))) >>> 0;
};
// returns first 2 chars of md5 hex string
const _md5_hex = function (str) {
    ...
    for (let offs = 0; offs < total; offs += 64)
    {
        ...
        for (let s = 0; s < 4; s++)
        {
            for (let i = s * 16, end = i + 16; i < end; i++)
            {
                ...
                const na = (b + _rotl(t >>> 0, S[s][i % 4])) >>> 0;
                [a, b, c, d] = [d, na, b, c];
            }
        }
...
L.CommonsPhotos = L.Photo.Cluster.extend({
    ...
    _requestData: function () {
        ...
        if (this._prevPoint && zoom == this._prevZoom && p.distanceTo(this._prevPoint) < this.options.updateMinPixelDistance)
            return;
        this._prevPoint = p;
        if (this._controller)
        {
